    ...
    <div>
      ...
      >
        ...
        <div className="absolute left-2 top-1/2 transform -translate-y-1/2 hidden md:flex flex-col z-40">
          {socialLinks.filter((link) => link.position === 'left').map((link, i) => (
        ...
        <div className="absolute right-2 top-1/2 transform -translate-y-1/2 hidden md:flex flex-col z-40">
          {socialLinks.filter((link) => link.position === 'right').map((link, i) => (
            <SocialButton key={i} {...link} />
          ))}
        </div>
      ...
      {/* Sticky Navbar */}
        <div className={`w-full z-50 transition-all duration-300 ease-in-out ${isSticky ? 'fixed top-0 bg-white dark:bg-gray-800 shadow-lg backdrop-blur-md' : ''}`}>
        <Navbar className={`shadow-md ${theme === 'dark' ? 'bg-gray-800 text-white' : 'bg-gradient-to-r from-yellow-300 via-yellow-500 to-black text-white'}`}>
          ...
          <Link to="/" className="flex items-center self-center whitespace-nowrap text-sm sm:text-xl font-semibold text-white">
            ...
            <span className="px-2 py-1 bg-gradient-to-r from-amber-400 to-orange-500 rounded-lg text-white font-bold">TellyKhabri</span>
            <span className="ml-2 hidden sm:inline text-orange-600 font-semibold">.Com</span>
          </Link>
{/* Desktop Search Bar */}
  ...
  <Navbar.Collapse>
    ...
    >
      <Link to="/projects">Videos</Link>
         ...
         {/* Mobile View: Social Icons below Navbar */}
